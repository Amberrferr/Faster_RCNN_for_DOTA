...
class ProposalQuadrangleOperator(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        #     y2 = bbox_deltas[:, i * 3, :, :].reshape((-1))
        #     x3 = bbox_deltas[:, i * 4, :, :].reshape((-1))
        #     y3 = bbox_deltas[:, i * 5, :, :].reshape((-1))
        #     x4 = bbox_deltas[:, i * 6, :, :].reshape((-1))
        #     y4 = bbox_deltas[:, i * 7, :, :].reshape((-1))
        #     x_stack = np.vstack((x1,x2,x3,x4))
        #     y_stack = np.vstack((y1,y2,y3,y4))
        #     xmin = np.amin(x_stack, axis=0).reshape((bbox_deltas.shape[0], -1, bbox_deltas.shape[2], bbox_deltas.shape[3]))
        #     ymin = np.amin(y_stack, axis=0).reshape((bbox_deltas.shape[0], -1, bbox_deltas.shape[2], bbox_deltas.shape[3]))
        #     xmax = np.amax(x_stack, axis=0).reshape((bbox_deltas.shape[0], -1, bbox_deltas.shape[2], bbox_deltas.shape[3]))
        #     ymax = np.amax(y_stack, axis=0).reshape((bbox_deltas.shape[0], -1, bbox_deltas.shape[2], bbox_deltas.shape[3]))
        #     temp_boxes[:, i * 0, :, :] = xmin
        #     temp_boxes[:, i * 1, :, :] = ymin
        #     temp_boxes[:, i * 2, :, :] = xmax
        #     temp_boxes[:, i * 3, :, :] = ymax
        # bbox_deltas = temp_boxes
...
@mx.operator.register("proposal_quadrangle")
class ProposalQuadrangleProp(mx.operator.CustomOpProp):
    def __init__(self, feat_stride='16', scales='(8, 16, 32)', ratios='(0.5, 1, 2)', output_score='False',
                 rpn_pre_nms_top_n='6000', rpn_post_nms_top_n='300', threshold='0.3', rpn_min_size='16', num_classes=2):
        super(ProposalQuadrangleProp, self).__init__(need_top_grad=False)
    ...
    def create_operator(self, ctx, shapes, dtypes):
        return ProposalQuadrangleOperator(self._feat_stride, self._scales, self._ratios, self._output_score,
