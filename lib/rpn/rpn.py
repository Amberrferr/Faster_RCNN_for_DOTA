...
from utils.image import get_image, tensor_vstack, get_image_quadrangle_bboxes, get_image_test, get_image_quadrangle_bboxes_test
import sys
import os
sys.path.append(os.path.dirname(os.path.realpath(__file__)))
from generate_anchor import generate_anchors
from bbox.bbox_transform import bbox_overlaps, bbox_transform, bbox_transform_quadrangle
...
def get_rpn_testbatch(roidb, cfg):
    ...
    imgs, roidb = get_image_test(roidb, cfg)
    im_array = imgs
...
def get_rpn_quadrangle_testbatch(roidb, cfg):
    """
        ...
        """
    # assert len(roidb) == 1, 'Single batch only'
    imgs, roidb = get_image_quadrangle_bboxes_test(roidb, cfg)
    im_array = imgs
    im_info = [np.array([roidb[i]['im_info']], dtype=np.float32) for i in range(len(roidb))]
    # im_idx = [np.array([roidb[i]['index']], dtype=np.int32) for i in range(len(roidb))]
...
# TODO test this quadrangle code
...
def assign_anchor(feat_shape, gt_boxes, im_info, cfg, feat_stride=16,
    ...
    if gt_boxes.size > 0:
        ...
        # overlaps (ex, gt)
        overlaps = bbox_overlaps(anchors.astype(np.float), gt_boxes[:,:4].astype(np.float))
...
# TODO add fpn quadrangle assign anchors
def assign_fpn_quadrangle_anchor(feat_shape, gt_boxes, im_info, cfg, feat_stride=16,
                  scales=(8, 16, 32), ratios=(0.5, 1, 2), allowed_border=0):
    """
    assign ground truth boxes to anchor positions
    :param feat_shape: infer output shape
